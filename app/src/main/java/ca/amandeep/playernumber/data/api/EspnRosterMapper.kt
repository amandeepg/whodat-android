...
internal fun EspnRosterResponse.toPlayers(league: EspnLeague): List<AnyPlayer> {
    ...
    val flattened = entries.flatMap { entry ->
...
private fun EspnRosterEntry.toPlayer(league: EspnLeague): AnyPlayer? {
    ...
    val resolvedNames = resolveNames(
        firstName = firstName,
        lastName = lastName,
    ...
    val positionValue = mapPositionForLeague(
        league = league,
...
private val suffixTokens = mapOf(
    "JR" to "Jr.",
    "SR" to "Sr.",
    "II" to "II",
    "III" to "III",
...
private fun fallbackPosition(
    ...
    val shortName = abbrev?.takeIf { it.isNotBlank() }
        ?: name
            ?.trim()
        ...
        ?: "UNK"
    val longName = name?.trim()?.takeIf { it.isNotBlank() }
